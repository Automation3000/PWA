...
// VAPID Configuration for Self-Hosted Web Push Protocol (Runs natively in GitHub / Node.js)
...
function loadSubscriptions() {
  try {
    ...
    console.error("Error reading subscriptions.json:", e);
  }
  return [];
}
...
// ─────────────────────────────────────────────────────────────
// SELF-HOSTED WEB PUSH NOTIFICATION API ROUTES (NO 3RD PARTY)
// ─────────────────────────────────────────────────────────────
...
app.post('/api/push/subscribe', async (req, res) => {
  try {
    ...
    if (existingIndex >= 0) {
      ...
      subs.push({ ...subRecord, createdAt: new Date().toISOString() });
    }
...
app.post('/api/push/unsubscribe', async (req, res) => {
  try {
    ...
    const existingIndex = subs.findIndex(s => s.endpoint === endpoint);
    if (existingIndex >= 0) {
      subs[existingIndex].status = 'unsubscribed';
...
// 4. Send Instant Test Push Notification
app.post('/api/push/send-test', async (req, res) => {
  try {
    ...
    const payload = JSON.stringify({
      ...
      body: message || 'Direct test notification from your self-hosted push engine.',
      url: url || '/index.html',
      icon: '/icon-192.png',
      badge: '/icon-72.png',
      timestamp: Date.now()
...
// 5. Broadcast Push Notification to ALL Active Subscribers
app.post('/api/push/broadcast', async (req, res) => {
  try {
    ...
    for (let i = 0; i < subs.length; i++) {
      ...
      let subObj;
      try {
        subObj = s.rawSubscription ? JSON.parse(s.rawSubscription) : { endpoint: s.endpoint, keys: s.keys };
      } catch (e) {
        subObj = { endpoint: s.endpoint, keys: s.keys };
      }
    ...
    return res.json({
      success: true,
      summary,
      stats: { sent, expired, failed, totalTargeted: sent + expired + failed }
    });
  } catch (error) {
